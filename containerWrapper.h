...
public:
  ...
  /*  explicit VectorWrapper( const value_type &arr ){
        size_t n = sizeof(arr) / sizeof(value_type);
        for(size_t i=0;i<n;i++ ){
            impl_.push_back(arr[i])
        }
    }
*/
    ...
    void insert(const value_type& element, std::size_t position) override{
        ...
        impl_.insert(std::next(impl_.begin(),position), element);
    }
    ...
    void erase(std::size_t position)override{
        impl_.erase( std::next(impl_.begin(),position) );
    }
    value_type count() const override{
        return std::accumulate(impl_.begin(),impl_.end(),value_type{},std::plus<value_type>{});
