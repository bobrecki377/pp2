...
public:
    ...
    Linux(std::string distribution) : distribution_(distribution){
        open_source_sympathizers_++;
        std::cout<<"+" << distribution_ << '\n';
    }
    ~Linux(){
    ...
    std::size_t add_user(std::string user,std::string pass);
    auto user(std::size_t id){
        return users_.at(id);
    }
    std::string user_home_directory(std::size_t id);
    std::optional<std::string> graphic_environment();
    void set_graphic_environment(std::string graph);
